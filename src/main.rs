    ...
    fn make_desc() -> Vec<InputElementDesc<'static>> {
        vec![
            ...
            InputElementDesc::default()
                ...
                .with_input_slot(0)
                .with_aligned_byte_offset(ByteCount(offset_of!(Self, color) as u64)),
        ]
    }
}
...
trait TypedBuffer {
    type ElementType;
    fn from_resource(resource: Resource, element_count: u32, element_size: ByteCount) -> Self;
}
...
impl TypedBuffer for VertexBuffer {
    ...
    fn from_resource(buffer: Resource, element_count: u32, element_size: ByteCount) -> Self {
...
impl HelloTriangleSample {
    pub fn new(hwnd: *mut std::ffi::c_void) -> Result<Self, HRESULT> {
        ...
        let mut factory = Factory::new(CreateFactoryFlags::Debug).expect("Cannot create factory");
        let adapter = Self::choose_adapter(&mut factory);

        let device = Device::new(&adapter).expect("Cannot create device");

        //let info_queue =
        //    Rc::new(InfoQueue::new(&device, None).expect("Cannot create debug info queue"));
        ...
        let command_list = device
            .create_command_list(CommandListType::Direct, &command_allocator, None)
...
"#,
        ...
        let input_layout = InputLayoutDesc::default().with_input_elements(&vertex_desc);
    ...
    pub fn draw(&mut self) {
        ...
        let current_buffer_index = self.swapchain.get_current_back_buffer_index();
        let current_buffer = self
            .swapchain
            .get_buffer(current_buffer_index)
        ...
        self.command_list
            .clear_render_target_view(rtv_handle, [0., 0.1, 0.8, 1.], &[]);
        self.command_list
            .set_render_targets(&[rtv_handle], false, None);
...
impl HelloTriangleSample {
    fn choose_adapter(factory: &mut Factory) -> Adapter {
        let mut adapters = factory.enum_adapters().expect("Cannot enumerate adapters");
        debug!("Found adapters:");
        for adapter in &adapters {
            let desc_struct = adapter.get_desc().expect("Cannot get adapter desc");
    ...
    fn create_default_buffer<T: TypedBuffer>(
        ...
        let size = ByteCount::from(init_data.len() * std::mem::size_of::<T::ElementType>());
    ...
    fn flush_command_queue(&mut self) {
        ...
        if self.fence.get_completed_value() < self.current_fence_value {
            ...
            self.fence
                .set_event_on_completion(self.current_fence_value, &event_handle)
    ...
    fn compile_shader(
        ...
        match result {
            ...
            Err(error) => {
                error!("Cannot compile shader: {}", &error.to_string());
                Err(error.to_string())
            }
        }
    }
...
fn main() {
    ...
    let mut sample = HelloTriangleSample::new(hwnd).expect("Cannot create renderer");
